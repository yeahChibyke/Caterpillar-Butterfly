...
contract Butterfly is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    /// >------> errors
    error Butterfly__NoMoreButterflies();
    error Butterfly__OnlyOneButterflyPerWallet();
    ...
    /// >------> events
    event ButterflyMinted(address indexed minter, uint256 nftId);

    /// >------> modifiers
    modifier hasNotMinted(address caller) {
        if (s_hasMinted[caller]) {
            revert Butterfly__OnlyOneButterflyPerWallet();
        }
        _;
    }
    ...
    modifier caterpillarsStillAvailable() {
        if (s_nftCounter == s_maxSupply) {
            revert Butterfly__NoMoreButterflies();
        }
        _;
    }
    ...
    function initialize(string memory butterflySvgImageUri) public initializer {
        __ERC721_init("Butterfly", "BNFT");
    ...
    function mintNft() external hasNotMinted(msg.sender) caterpillarsStillAvailable returns (string memory) {
        ...
        emit ButterflyMinted(msg.sender, nftCounter);

        return string(abi.encodePacked("There are ", Strings.toString(s_nftCounter), "Butterfly NFTs remaining!"));
    }
    ...
    function nftURI(uint256 nftId) public view virtual returns (string memory) {
        ...
        string memory imageURI = s_butterflySvgImageUri;
    ...
    function getButterflySvg() external view returns (string memory) {
        return s_butterflySvgImageUri;
    }
}
